...
package body CBOR_Codec is
   ...
   begin
      ...
      elsif Value <= Integer (Unsigned_8'Last) then
         Encoding_Byte.Value := Additional_8Bit;
         Write (Encoding_Byte.Val);
         Write (Unsigned_8 (Value));
      elsif Value <= Integer (Unsigned_16'Last) then
         Encoding_Byte.Value := Additional_16Bit;
         Write (Encoding_Byte.Val);
         Write (Unsigned_8 (Value / 2**8));
         Write (Unsigned_8 (Value mod 2**8));
      elsif Value <= Integer'Last then
         Encoding_Byte.Value := Additional_32Bit;
         Write (Encoding_Byte.Val);
         Write (Unsigned_8 ((Value / 2**24) mod 2**8));
         Write (Unsigned_8 ((Value / 2**16) mod 2**8));
         Write (Unsigned_8 ((Value / 2**8)  mod 2**8));
         Write (Unsigned_8 (Value mod 2**8));
      end if;
