package body STM32GD.SPI is

   procedure Init is
   begin
      SPI.CR1 := (MSTR => 1, SSI => 1, SSM => 1, BR => 0, DFF => 0,
   ...
   procedure Send (Data : in Byte_Array) is
   begin
      for D of Data loop
         Send (D);
      end loop;
   ...
   procedure Receive (Data : out Byte_Array) is
   begin
   ...
   procedure Transfer (Data : in out Byte_Array) is
   begin
      for I in Data'Range loop
         while SPI.SR.TXE = 0 loop
            null;
