...
package body STM32GD.USART.Peripheral is
   ...
   begin
      USART_Periph.BRR.DIV_Fraction := UInt4 (((Frac_Divider * 16) + 50) / 100 mod 16);
      USART_Periph.BRR.DIV_Mantissa := UInt12 (Int_Divider / 100);
   ...
   begin
      if Count > 0 then
         ...
         if USART = USART_1 then
            DMA1_Periph.CPAR2 := W (USART_Periph.TDR'Address);
            DMA1_Periph.CMAR2 := W (Data'Address);
            DMA1_Periph.CNDTR2.NDT := UInt16 (Count);
            USART_Periph.ICR.TCCF := 1;
            DMA1_Periph.CCR2.MINC := 1;
            DMA1_Periph.CCR2.DIR := 1;
            DMA1_Periph.CCR2.EN := 1;
            while USART_Periph.ISR.TC = 0 loop
               null;
            end loop;
            DMA1_Periph.CCR2.EN := 0;
         elsif USART = USART_2 then
            DMA1_Periph.CPAR4 := W (USART_Periph'Address);
            DMA1_Periph.CMAR4 := W (Data'Address);
            DMA1_Periph.CNDTR4.NDT := UInt16 (Count);
            USART_Periph.ICR.TCCF := 1;
            DMA1_Periph.CCR4.MINC := 1;
            DMA1_Periph.CCR4.DIR := 1;
            DMA1_Periph.CCR4.EN := 1;
            while USART_Periph.ISR.TC = 0 loop
               null;
            end loop;
            DMA1_Periph.CCR4.EN := 0;
         end if;
      end if;
   end DMA_Transmit;
   ...
   begin
      ...
      if USART = USART_1 then
         DMA1_Periph.CPAR3 := W (USART_Periph.RDR'Address);
         DMA1_Periph.CMAR3 := W (RX_DMA_Buffer'Address);
         DMA1_Periph.CNDTR3.NDT := RX_DMA_Buffer'Length;
         DMA1_Periph.CCR3.MINC := 1;
         DMA1_Periph.CCR3.CIRC := 1;
         DMA1_Periph.CCR3.DIR := 0;
         DMA1_Periph.CCR3.EN := 1;
      elsif USART = USART_2 then
         DMA1_Periph.CPAR5 := W (USART_Periph.RDR'Address);
         DMA1_Periph.CMAR5 := W (RX_DMA_Buffer'Address);
         DMA1_Periph.CNDTR5.NDT := RX_DMA_Buffer'Length;
         DMA1_Periph.CCR5.MINC := 1;
         DMA1_Periph.CCR5.CIRC := 1;
         DMA1_Periph.CCR5.DIR := 0;
   ...
   begin
      Count := 0;
      DMA_Pos := RX_DMA_Buffer'Length - Integer (DMA1_Periph.CNDTR3.NDT) + RX_DMA_Buffer'First;
   ...
   begin
      loop
         if USART = USART_1 then
            ...
            DMA_Pos := RX_DMA_Buffer'Length - Integer (DMA1_Periph.CNDTR5.NDT) + RX_DMA_Buffer'First;
         end if;
